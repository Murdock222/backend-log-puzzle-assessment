...
def read_urls(filename):
    ...
    with open(filename, 'r') as log_file:
        ...
        for url in log_items:
            ...
            puzzle_url.append(url_result[0])
    url_list =  create_urls(puzzle_url)
    url_list = list(set(url_list))
    sorted_urls = sorted(url_list, key=return_last_word)
    
    def extract_host_name(url):
        """Returns host name from an url"""
    host = url_result = re.findall(r'GET (\S+) HTTP', url)
    return sorted_urls
...
def add_prefixes(filename, host_list):
    """Add server prefixes to urls and host list"""
